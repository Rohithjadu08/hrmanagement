...
export default function HrChatHistoryPage() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white tracking-tight">AI Chat History</h1>
        <p className="mt-1 text-sm text-[#94A3B8]">Review your past conversations with the HR AI Assistant.</p>
      </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredConversations.map(conv => (
              <div 
                key={conv.id} 
                onClick={() => nav(`/hr/chat?id=${conv.id}`)}
